...
impl DBConnection {
    pub fn get_many_from_db<T: for<'de> Deserialize<'de>>(&self,source:&str, filter: impl Into<Option<Document>>) -> Result<Vec<T>, ApiResponse> {
    ...
    pub fn get_one_from_db<T: for<'de> Deserialize<'de>>(&self,source:&str, filter: impl Into<Option<Document>>) -> Result<T, ApiResponse> {
    ...
    pub fn add_one_to_db(&self,source:&str, document: Document) -> Result<i32, ApiResponse> {
    ...
    pub fn delete_from_db(&self,source:&str, id: i32) -> Result<i8, ApiResponse> {
        match self.collection(source).find_one_and_delete(doc! {"_id":id}, None) {
            Ok(_) => Ok(1),
            Err(_) => Err(ApiResponse::not_found())
        }
    }
    pub fn delete_many_from_db(&self,source:&str, filter: Document) -> Result<i8, ApiResponse> {
        match self.collection(source).delete_many(filter, None) {
            Ok(_) => Ok(1),
            Err(_) => Err(ApiResponse::not_found())
        }
    }
    pub fn replace_in_db<T: for<'de> Deserialize<'de>>(&self,source:&str, id: i32, document: Document) -> Result<T, ApiResponse> {
    ...
    pub fn update_in_db<T: for<'de> Deserialize<'de>>(&self,source:&str, id: i32, document: Document) -> Result<T, ApiResponse> {
